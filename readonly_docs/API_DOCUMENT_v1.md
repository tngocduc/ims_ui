...
---
 ...
 #### Get Tenant VietQR Configuration (Admin)
 ```
 GET /api/v1/core/tenants/{tenant_id}/vietqr-config
 ```
 ...
 **Response (200):**
 ```json
 {
   ...
   "data": {
     ...
     "vietqr_account_name": "IMS VENDING",
     "has_vietqr_config": true
   }
 }
 ```
 ...
 #### Update Tenant VietQR Configuration (Admin)
 ```
 PUT /api/v1/core/tenants/{tenant_id}/vietqr-config
 ```
 ...
 **Request:**
 ```json
 {
   "vietqr_bank": "VIETCOMBANK",
   "vietqr_account_number": "1234567890",
   "vietqr_account_name": "MY COMPANY"
 }
 ```
 ...
 **Parameters:**
 ...
 |-----------|------|----------|-------------|
 | `vietqr_bank` | string | Yes | Vietnamese bank name/alias (e.g., VIETCOMBANK, VIETINBANK, BIDV, AGRIBANK, TECHCOMBANK, MBBANK) or 6-digit bank BIN |
 | `vietqr_account_number` | string | Yes | Bank account number |
 | `vietqr_account_name` | string | Yes | Account holder name displayed in banking apps |
...
**Response (200):** Same format as Administration Sales Report
 ...
 #### Get Tenant VietQR Configuration (Tenant Admin)
 ```
 GET /api/v1/core/tenant/vietqr-config
 ```
 ...
 **Response (200):**
 ...
 #### Update Tenant VietQR Configuration (Tenant Admin)
 ```
 PUT /api/v1/core/tenant/vietqr-config
 ```
...
**Note:** VietQR payment settings are now configured per-tenant via the VietQR Configuration API endpoints, not via Django settings. See [Tenant VietQR Configuration](#get-tenant-vietqr-configuration-admin) and [Tenant VietQR Configuration (Tenant Admin)](#get-tenant-vietqr-configuration-tenant-admin).
...
`qr_data` is a VietQR/EMVCo bank transfer payload generated from **tenant configuration** (configured via `/api/v1/core/tenants/{tenant_id}/vietqr-config`):
...
If tenant VietQR is not configured, fallback defaults are used: `VIETCOMBANK`, `0000000000`, `IMS VENDING`.
...
## Data Models
 ...
 | vietqr_bank | String(50) | Vietnamese bank name/alias or 6-digit BIN (e.g., VIETCOMBANK, 970436) |
 | vietqr_account_number | String(50) | Bank account number |
 | vietqr_account_name | String(255) | Account holder name displayed in banking apps |
